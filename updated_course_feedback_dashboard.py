...
def main():
    ...
    # NEW: Client type filter - if column exists
    ...
    # NEW: Course status filter - if column exists
    if 'course_status' in df.columns and df['course_status'].notna().any():
        all_statuses = ["All Statuses"] + sorted(df['course_status'].dropna().unique().tolist())
        selected_status = st.sidebar.selectbox("Course Status", all_statuses)
        
        if selected_status != "All Statuses":
            filtered_df = filtered_df[filtered_df['course_status'] == selected_status]
    ...
    if "Statistical Analysis" in tab_list:
        with tabs[tab_list.index("Statistical Analysis")]:
            ...
            if len(numeric_cols) < 2 or filtered_df[numeric_cols].count().sum() < 10:  # Arbitrary threshold
                ...
                categorical_vars = [col for col in ["course_category", "client_sector", "country", "client_type", "course_status"] 
    ...
    if "Sentiment Analysis" in tab_list:
        with tabs[tab_list.index("Sentiment Analysis")]:
            ...
            if not sentiment_cols:
                ...
                # New: Sentiment by client type if available
                ...
                # New: Sentiment by course status if available
                if 'course_status' in filtered_df.columns and filtered_df['course_status'].notna().any():
                    sentiment_by_status = filtered_df.groupby('course_status')[sentiment_cols].mean().reset_index()
                    
                    fig = px.bar(sentiment_by_status, x='course_status', y=sentiment_cols,
                               barmode='group',
                               title="Average Sentiment by Course Status",
                               labels={'value': 'Average Sentiment', 'course_status': 'Course Status', 'variable': 'Sentiment Type'},
                               color_discrete_map={'positive_sentiment': '#2E8B57', 'improvement_sentiment': '#B22222'})
                    st.plotly_chart(fig, use_container_width=True)
                ...
                if filtered_df[topic_col].dropna().shape[0] > max(10, n_topics*2):  # Ensure enough data
                    try:
                        ...
                        if topics_words is None:
                            ...
                            # New: Topic Distribution by Client Type if available
                            if 'client_type' in filtered_df.columns and filtered_df['client_type'].notna().any():
                            ...
                            # New: Topic Distribution by Course Status if available
                            if 'course_status' in filtered_df.columns and filtered_df['course_status'].notna().any():
                                st.subheader("Topic Distribution by Course Status")
                                
                                # Get topic distribution for each course status
                                course_statuses = filtered_df['course_status'].dropna().unique()
                                
                                if len(course_statuses) > 1:  # Only proceed if we have multiple statuses
                                    status_topic_data = []
                                    
                                    for status in course_statuses:
                                        status_df = filtered_df[filtered_df['course_status'] == status]
                                        ...
                                        if len(status_texts) > 5:  # Ensure enough texts
                                            try:
                                                ...
                                                st.error(f"Error processing course status {status}: {e}")
                                    
                                    if status_topic_data:
                                        status_topic_df = pd.DataFrame(status_topic_data)
                                        fig = px.bar(status_topic_df, x='Topic', y='Proportion', 
                                                   color='Course Status', barmode='group',
                                                   title="Topic Distribution by Course Status")
                                        st.plotly_chart(fig, use_container_width=True)
                                    else:
                                        st.info("Not enough data for topic distribution by course status")
                    except Exception as e:
            ...
            if len(valid_features) < 2 or filtered_df[valid_features].dropna().shape[0] < 10:
                ...
                try:
                    ...
                    if segmented_df is None or 'cluster' not in segmented_df.columns:
                        ...
                        # New: Client type distribution within each segment if available
                        ...
                        # New: Course status distribution within each segment if available
                        if 'course_status' in segmented_df.columns and segmented_df['course_status'].notna().any():
                            st.subheader("Course Status Distribution by Segment")
                            
                            # Calculate the count and percentage of each course status in each segment
                            course_status_segment = pd.crosstab(
                                segmented_df['cluster'], 
                                segmented_df['course_status'], 
                                normalize='index'
                            ).reset_index() * 100
                            
                            # Rename cluster to segment
                            course_status_segment['Segment'] = [f"Segment {i+1}" for i in range(n_clusters)]
                            
                            # Reshape for plotting
                            course_status_segment_melted = pd.melt(
                                course_status_segment, 
                                id_vars=['cluster', 'Segment'], 
                            ...
                            # Create stacked bar chart
                            fig = px.bar(
                                course_status_segment_melted, 
                                x='Segment', 
                                y='Percentage', 
                                color='Course Status',
                                title="Course Status Distribution within Each Segment (%)",
                                labels={'Percentage': '% of Segment'}
                            )
